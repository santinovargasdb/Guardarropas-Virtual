...
/** Loads a File into a decoded HTMLImageElement via a data URL. */
...
/**
 * Core chroma-key. Returns a canvas with near-background pixels made transparent,
 * or `null` when the background is not a removable flat/light surface. With
 * `force = true` the flat/light and ratio guards are skipped so an interactive
 * tool can always produce a tunable preview (the user decides if it looks good).
 */
function buildCutoutCanvas(
  ...
  if (!force) {
    // Auto mode: only proceed when the background is light AND low-variance (flat).
    const lum = 0.299 * br + 0.587 * bg + 0.114 * bb;
    let variance = 0;
    for (const c of corners) {
      variance += Math.abs(px[c] - br) + Math.abs(px[c + 1] - bg) + Math.abs(px[c + 2] - bb);
    }
    if (lum < 180 || variance > 120) return null;
  }

  // Knock out pixels close to the background colour.
  const cutoff = threshold * 3; // distance summed over 3 channels
  let removed = 0;
  ...
  if (!force) {
    // Auto mode: bail if almost nothing / almost everything was removed.
...
/**
 * Best-effort background remover for photos on a flat/light background. Returns a
 * transparent PNG File, or `null` (never rejects) so the caller can fall back to
 * the compressed JPEG. Pass `force = true` to skip the auto-detection guards.
 */
    ...
    .then(
      (img) =>
        new Promise<File | null>((resolve) => {
          const canvas = buildCutoutCanvas(img, maxSize, threshold, force);
...
/**
 * Interactive preview variant — returns a transparent PNG data URL (or `null`)
 * for the in-form background-remover assistant. Forces removal by default so the
 * user can tune the threshold and judge the result live.
 */
export function previewFlatBackground(
  file: File,
  threshold = 38,
  force = true,
  maxSize = 420,
